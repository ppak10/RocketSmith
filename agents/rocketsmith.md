...
│       ├── <flight_name>.json
│       └── ...
├── component_tree.json        ← DFAM output, authoritative parts list
...
The `component_tree.json` at the project root is the single source of truth for which parts exist and how they're derived from OpenRocket components. The `design-for-additive-manufacturing` skill writes it; the `generate-structures` skill reads it for Pass 1 (base geometry) and the `modify-structures` skill reads it for Pass 2 (detail features); the `mass-calibration` skill reads `component_to_part_map` from it during the calibration phase.
...
- `openrocket_generate_tree(rocket_file_path="<project_dir>/H100W.ork")` — absolute
- `cadsmith_run_script(script_path="<project_dir>/parts/cadsmith/nose_cone.py", out_dir="<project_dir>/parts/step")` — absolute
- `cadsmith_generate_preview(step_file_path="<project_dir>/parts/step/nose_cone.step")` — generates STL, PNG, GIF in parallel
