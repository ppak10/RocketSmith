...
description: >
  ...
  <example>
  Context: User wants to build a complete rocket end-to-end.
  user: 'Build me a rocket'
  assistant: 'I'll use the rocketsmith agent to design and simulate the rocket in OpenRocket, confirm stability, then generate build123d scripts for each part and export STEP files.'
  <commentary>Full build requires OpenRocket simulation followed by CAD part generation via build123d.</commentary>
  </example>
  <example>
  ...
  <example>
  Context: User wants STEP files from a confirmed design.
  user: 'Generate the STEP files for my rocket'
  assistant: 'I'll read the confirmed dimensions from the OpenRocket design, write build123d scripts for each part, execute them, and export STEP files to the workspace parts/ folder.'
  <commentary>STEP file generation requires extracting dimensions from the .ork file and running parametric build123d scripts.</commentary>
  </example>
model: sonnet
color: red
---
...
> **RULE — Every body tube joint MUST have a coupler. No exceptions.**
> Any place where two body tube sections meet (nose-to-upper, upper-to-middle, middle-to-lower) requires a
> coupler: an `inner-tube` child of the forward section, OD = forward tube ID, wall 2–3 mm, length 1.0–1.5×
> body diameter, positioned with `axial_offset_method="bottom"` and `axial_offset_m=+(coupler_length/2)` so
> half protrudes into the aft section. Couplers must also be modelled as CAD parts in `upper_airframe.py`
> (or whichever section owns them). Before proceeding to CAD generation, verify every joint has a coupler.
