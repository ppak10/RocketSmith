...
``openrocket_component`` with action='read') and annotates each component's ``agent``
...
def annotate_dfam(
    ...
    Args:
        tree: A ComponentTree generated by ``openrocket_component`` (action='read').
        fusion_overrides: Optional dict of fusion decision overrides:
            ...
            - ``modifications``: list of modification dicts to attach to the
              body tube's agent annotation as ``dfam_modifications``.  Each
              dict should specify ``kind`` (e.g. ``"radial_through_holes"``),
              placement, and sizing fields consumed by the
              ``modify-structures`` skill.
...
def _annotate_component(
    ...
    if comp.type == "NoseCone":
        ...
        mods = overrides.get("modifications")
        extra_kw: dict[str, object] = {}
        if mods:
            extra_kw["dfam_modifications"] = mods
        _annotate(
            comp,
            fate=Fate.PRINT,
