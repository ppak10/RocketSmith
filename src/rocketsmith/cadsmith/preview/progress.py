...
``gui/progress/<part_name>.json`` so multiple parts can generate
previews concurrently without conflicts. The GUI watcher polls the
``gui/progress/`` directory to display live progress.
"""
...
class PreviewProgress:
    """Manages a per-part progress file in gui/progress/."""

    def __init__(self, project_dir: Path, part_name: str, outputs: list[str]) -> None:
        from rocketsmith.gui.layout import PROGRESS_DIR
